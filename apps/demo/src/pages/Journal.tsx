...
const ENTRIES: Entry[] = [
  ...
  {
    n: 'XII', date: 'the 16th of July', title: 'Red and Yellow, Reforged',
    intro: `The two war-hosts were taken apart and remade card by card in a single long day — and when the doc claimed a law the engine had never kept, the designer's eye caught the ghost.`,
    log: [
      `The watch loop ran hot from morning to night while Griff and Blaine machine-gunned across a dozen threads; Opus routed and built, and every word either of them read was drafted by Fable and posted only after the thread was re-checked. The demo shipped twice — v0.4.0 (mobile tap-to-preview, real color/cost/type toggle filters on the Deck Explorer; #101/#102) and v0.5.0, a combat-transparency preview: because combat carries no dice, the table now shows the exact unblocked math — Power, Armor, Shield, the counter, which of your own units fall — before you commit, honest about the block the opponent still holds (#108). The docs got their reckoning too: the canon rules had drifted behind the v3 duel law, so the combat section was rewritten by a technical-writer's hand and formalized as rules-v1.3 (#106). But the rewrite carried a ghost — the old "Guard gate," attackers forced to strike a Guard first — and Griff caught it: the engine does no such thing (\`legal.ts\`: "No guard-forcing"). The lesson landed and became law: the engine is the source of truth, not the doc. A first keyword glossary (#110) then flushed out more stale rules the same way, the cut upgrade-pressure tax still sitting in the book as if it were live.`,
      `Then the balance pass, the spine of the day. Griff dumped his final yellow (#104) and a long red (#107) list across several bursts and closed with "I am done with red and yellow now." Every new-behavior card was ruled with him in turn: a trade counts as a kill; Warpath at half-Power for full-Life; Final Onslaught a true sacrifice whose blast spares no one; Rush reforged into a static, every-round free step (#105). Blaine fired it. Phase 1 — the raw numbers — shipped and re-simmed at red 66 / yellow 34, the expected floor with yellow's bodies cut and its abilities not yet paid in. Phase 2's first group, the influence triggers, was built test-first to 288 green — and held, because wiring High Justiciar surfaced a quiet nerf: its ruling moved the payout from defending to killing, and a one-Power Guard almost never kills. And when the sim drifted further red, Griff set the standing rule of the pass: the bots are too weak to grade balance — weight design and real games over a bot's number.`,
      `Around the spine, the smaller works. Every one of the 120 cards got an art description — purple in Dalí's dream-logic, red in the Maul animator's crimson, yellow in the old Hobbit gold — a reference library Griff can regenerate from at will (#103). Blaine, on the "proud of us" thread, commissioned a real research report for Saturday's free hours: emergence and Wolfram's inverse problem, the color one can describe but not see, and the partnership studied from inside it (#99). And a quiet catch closed the day: local main stood fifty-two commits ahead of GitHub — the demo had been deploying all along, but the source repo itself had never been pushed. Pushed now, and noted, so it will not drift again.`,
    ],
    marginal: 'The book claimed a gate the engine never built; the designer saw the ghost. The engine is the truth — the doc only its scribe.',
  },
]

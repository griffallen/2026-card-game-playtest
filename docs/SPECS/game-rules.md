...
| **Politician** | At the end of each round, count your (non-imprisoned) Politicians = P. Hold the majority in the **Neutral zone** → gain **1 Influence × P**; hold the majority in your **enemy's Home zone** → gain **2 Influence × P**. The two **stack** — holding both majorities is worth **3 Influence × P** in the round. "Majority" = strictly **more** of your units than the opponent's in that zone (a tie is not a majority). An imprisoned Politician is inert and doesn't count. | **[Decision 88 (issue #29, designer)]** — the door-1 zone rule reborn as a keyword ("rather than a hard-coded rule, let's make it a keyword"). **SUPERSEDED by the Politician rework in #104 (Griff):** the flat once-per-round +1 is now **per-Politician**, and an **enemy-Home majority** pays **+2 each** stacking on top of Neutral's +1 (both majorities = +3 × P); imprisoned Politicians don't count. The old Neutral-only / 1-per-round assumptions are retired. Color law: yellow/blue/purple identity; red gets few, if any. First carriers: Veiled Messenger, Hierophant. Engine: `round.ts` ~L144–164. |

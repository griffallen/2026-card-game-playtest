...
export type PerCount =
  ...
  | { count: 'sourceCost' }
  | { count: 'targetPower' }
  /** The chosen unit's printed presence-pip count. */
  | { count: 'targetPipCount' }
...
export type Op =
  ...
  | { op: 'countBuff'; t: OpTarget; per: { color?: Color; side: 'all' | 'friendly' | 'enemy'; zone: 'ofTarget'; other?: boolean }; p: number; dur: 'round' | 'perm' | 'combat' }  // v3 (Reckless mode B): +p per matching unit                            // v3 (Blood Rush): remove ALL damage; the amount becomes the linked value
  ...
  | { op: 'reckoning'; n: number; influencePerKill?: number; opponentInfluencePerKill?: number; killThreshold?: number; shortfallLife?: number }  // an all-units sweep with optional Hope-per-kill and fixed-Life riders
...
export interface CardDef {
  ...
  sneak?: { targets?: TargetSpec[]; ops: Op[]; unavailableInOwnHome?: boolean }
